...
def backward(activations, clipping = True):
    ...
    dWex, dWy = np.zeros_like(Wex), np.zeros_like(Wy)
    ...
    if clipping:
        ...
        for dparam in [dWex, dWf, dWi, dWo, dWc, dbf, dbi, dbo, dbc, dWy, dby]:
            np.clip(dparam, -5, 5, out = dparam)

    gradients = (dWex, dWf, dWi, dWo, dWc, dbf, dbi, dbo, dbc, dWy, dby)
...
if option == 'train':
    ...
    mWex, mWy = np.zeros_like(Wex), np.zeros_like(Wy)
    ...
    while True:
        ...
        if n % 100 == 0:
            sample_ix = sample((h_prev, c_prev), inputs[0], 200)
            txt = ''.join(ix_to_char[ix] for ix in sample_ix)
            print('----\n %s \n----' % (txt,))

        # forward seq_length characters through the net and fetch gradient
        loss, activations, memory = forward(inputs, targets, (h_prev, c_prev))
        gradients = backward(activations)

        h_prev, c_prev = memory
        dWex, dWf, dWi, dWo, dWc, dbf, dbi, dbo, dbc, dWy, dby = gradients
        smooth_loss = smooth_loss * 0.999 + loss * 0.001
        if n % 100 == 0:
            print('iter %d, loss: %f' % (n, smooth_loss))  # print progress

        # perform parameter update with Adagrad
        for param, dparam, mem in zip([Wf, Wi, Wo, Wc, bf, bi, bo, bc, Wex, Wy, by],
                                      [dWf, dWi, dWo, dWc, dbf, dbi, dbo, dbc, dWex, dWy, dby],
                                      [mWf, mWi, mWo, mWc, mbf, mbi, mbo, mbc, mWex, mWy, mby]):
            mem += dparam * dparam
    ...
    memory = (h_prev, c_prev)

    loss, activations, _ = forward(inputs, targets, memory)
    gradients = backward(activations, clipping = False)
    dWex, dWf, dWi, dWo, dWc, dbf, dbi, dbo, dbc, dWy, dby = gradients

    for weight, grad, name in zip([Wf, Wi, Wo, Wc, bf, bi, bo, bc, Wex, Wy, by],
                                  [dWf, dWi, dWo, dWc, dbf, dbi, dbo, dbc, dWex, dWy, dby],
                                  ['Wf', 'Wi', 'Wo', 'Wc', 'bf', 'bi', 'bo', 'bc', 'Wex', 'Wy', 'by']):
